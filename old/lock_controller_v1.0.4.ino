...
void setup() {
  ...
  server.on("/", HTTP_GET, []() {
    ...
    html += "<script>\n";
    html += "function setPw(){let pw=document.getElementById('pw').value;fetch('/set_password?pw='+pw).then(r=>r.text()).then(t=>{alert(t);loadPwList();});}";
    html += "function addTempPw(){let pw=document.getElementById('tpw').value;let expire=document.getElementById('expire').value;let count=document.getElementById('count').value;let url='/add_temp_pw?pw='+pw;if(expire)url+='&expire='+expire;if(count)url+='&count='+count;fetch(url).then(r=>r.text()).then(t=>{alert(t);loadPwList();});}";
    html += "function removeTempPw(pw){if(confirm('確定移除?'))fetch('/remove_temp_pw?pw='+pw).then(r=>r.text()).then(t=>{alert(t);loadPwList();});}";
    html += "function clearAllTempPw(){if(confirm('確定清除所有臨時密碼?'))fetch('/clear_temp_pw').then(r=>r.text()).then(t=>{alert(t);loadPwList();});}";
    html += "function loadPwList(){fetch('/list_passwords').then(r=>r.json()).then(j=>{let html='<b>主密碼:</b> '+j.main+'<br><b>臨時密碼:</b><ul>';for(let tp of j.temps){html+='<li>'+tp.pw+' ';if(tp.expire)html+='(到期:'+new Date(tp.expire*1000).toLocaleString()+') ';if(tp.count>=0)html+='(剩餘:'+tp.count+') ';html+='<button onclick=\\'removeTempPw(\\''+tp.pw+'\\')\\'>移除</button></li>';}html+='</ul>';document.getElementById('pwlist').innerHTML=html;});}";
    html += "loadPwList();\n";

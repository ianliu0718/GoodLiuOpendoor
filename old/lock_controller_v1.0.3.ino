...
void setup() {
  ...
  server.on("/", HTTP_GET, []() {
    ...
    html += "<input id='count' type='number' placeholder='次數'><button onclick=\"addTempPw()\">新增臨時密碼</button>";
    html += "<script>";
  ...
  for (auto it = tempPasswords.begin(); it != tempPasswords.end(); ) {
    ...
    if (it->pw == input) {
      if ((it->expireTime==0 || now < it->expireTime) && (it->remainCount==-1 || it->remainCount>0)) {
        ...
        if (it->remainCount==0) it = tempPasswords.erase(it);
        else ++it;

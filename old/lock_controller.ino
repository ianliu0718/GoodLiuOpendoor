...
// WiFi 設定
const char* ssid = "HiHiLive2323";
const char* password = "ianian520";
...
String correct_password = "1234"; // 可自行更改
...
void showResult(char result) {
  ...
  if (result == 'O') {
    // 自訂 O 圖案（以 8 位數顯示器為例，顯示在最右一位）
    lc.setRow(0, 0, B00011101); // 你可根據需求調整顯示位置
  } else if (result == 'I') {
    lc.setRow(0, 0, B00000110); // I 字
  } else {
...
void setup() {
  ...
  // 顯示歡迎訊息 （只支援 0-9、E、H、L、P、-、空白等）
  lc.setChar(0, 7, 'H', false); // H
  lc.setRow(0, 6, B00000110);   // I
  ...
  server.on("/set_password", HTTP_GET, []() {
    if (!server.hasArg("pw")) { server.send(400, "text/plain", "缺少pw"); return; }
